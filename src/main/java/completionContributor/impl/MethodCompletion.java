...
public class MethodCompletion extends BasicCompletion {
    ...
    public List<LookupElementBuilder> getLookupElement(PsiMethod currentMethod) {
        ...
        if (TYPE_CONSTANT.LIST.equals(returnTypeName)) {
            returnList.addAll(lookupElementBuilderList(returnClass, currentMethod, TYPE_CONSTANT.LIST_TYPE_LIST, codeStr, endStr));
        } else if (TYPE_CONSTANT.MAP.equals(returnTypeName)) {
            returnList.addAll(lookupElementBuilderList(returnClass, currentMethod, TYPE_CONSTANT.MAP_TYPE_LIST, codeStr, endStr));
        } else if (TypeUtil.isObject(returnTypeName)) {
            //TODO 兼顾低版本，不使用List.of
            returnList.addAll(lookupElementBuilderList(returnClass, currentMethod, Arrays.asList(returnTypeName), codeStr, endStr));
        }
        return returnList;
    }

    private List<LookupElementBuilder> lookupElementBuilderList(PsiClass returnClass, PsiMethod currentMethod, List<String> typeList, String codeStr, String endStr) {
        //TODO 优化类导入
