...
public abstract class BaseExport {
    private final List<String> headerList = List.of("序号", "字段", "类型", "注释", "默认值", "不可为null", "主键", "自增");
    ...
    protected void createSheet(XSSFWorkbook workbook, DasObject dasTable) {
        ...
        for (DasColumn column : DasUtil.getColumns(dasTable)) {
            ...
            ExcelUtil.createCell(row, 5, column.isNotNull() ? Common.HOOK_UP : Common.BLANK_STRING, commonStyle);
            ExcelUtil.createCell(row, 6, DasUtil.isPrimary(column) ? Common.HOOK_UP : Common.BLANK_STRING, commonStyle);
            ExcelUtil.createCell(row, 7, DasUtil.isAutoGenerated(column) ? Common.HOOK_UP : Common.BLANK_STRING, commonStyle);
            //自适应高度
