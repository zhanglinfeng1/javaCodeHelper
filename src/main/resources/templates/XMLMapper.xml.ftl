...
<#list xmlMapper.resultMaps as resultMap>
    ...
    <#list resultMap.columnList as column>
        <#if column.sqlName?upper_case =="ID"><id column="${column.sqlName}" property="${column.javaName}"  javaType="${fun.javaTypeTrun(column.javaType)}"${fun.typeHandler(column)}/><#else><result column="${column.sqlName}" property="${column.javaName}"  javaType="${fun.javaTypeTrun(column.javaType)}"${fun.typeHandler(column)}/></#if>
    </#list>
...
${operation.cdata!}
    ...
    <${lib.operation2Sql(operation.name)} id="${operation.name}" ${lib.mapperResult(operation)}${lib.timeout(operation)} <#if operation.useGeneratedKeys??>useGeneratedKeys="${operation.useGeneratedKeys}"<#if operation.keyProperty??> keyProperty="${operation.keyProperty}"</#if></#if>${lib.resultSetType(operation)}${lib.fetchSize(operation)} ${lib.useCache(operation)}>
